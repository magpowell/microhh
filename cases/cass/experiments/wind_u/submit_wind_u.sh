#!/bin/bash
# Submit CASS wind_u experiment runs.
# For each u value: 2 jobs (2stream + raytracer), 4 reps each on 1 node.
# u=0.0 included (swlspres=uflux, uflux=0) for apple-to-apple comparison.
set -euo pipefail

SCRATCH=${SCRATCH:-/pscratch/sd/m/mpowell}
EXP="$SCRATCH/CASS_LES/experiments/wind_u"
SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"

mkdir -p "$SCRATCH/CASS_LES/logs"

# u values and their labels — must match setup_wind_u.py u_label()
# e.g. 0.0 -> u_0p0,  2.5 -> u_2p5,  10.0 -> u_10p0
U_VALUES=(0.0 2.5 5.0 7.5 10.0)
...
for u in "${U_VALUES[@]}"; do
    ...
    sbatch \
        --constraint=gpu \
        --time=10:00:00 \
        --job-name="wu${label}_2s" \
        --export=ALL,SIM_DIRS="$SIM_DIRS" \
        "$SCRIPT_DIR/sbatch_wind_u.sh"
    echo "Submitted u=${u} m/s 2stream"
    ...
    sbatch \
        ...
        --time=22:00:00 \
        --job-name="wu${label}_rt" \
        --export=ALL,SIM_DIRS="$SIM_DIRS" \
        "$SCRIPT_DIR/sbatch_wind_u.sh"
    echo "Submitted u=${u} m/s raytracer"
done
