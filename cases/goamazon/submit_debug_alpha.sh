#!/bin/bash
# Set up and submit goamazon DEBUG runs on Empire AI Alpha.
...
echo "Setting up goamazon debug runs (SCRATCH=$SCRATCH)..."
...
for RT in 2stream raytracer; do
    SIM_DIR="$SCRATCH/GOAMAZON_LES/debug/$RT/rep_01"
    jid=$(sbatch --parsable "${COMMON[@]}" \
        --job-name="dbg_goam_${RT}" \
    ...
    echo "Submitted goamazon debug $RT: job $jid"
done
