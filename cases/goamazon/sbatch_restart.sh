...
#SBATCH --output=/pscratch/sd/m/mpowell/GOAMAZON_LES/logs/goamazon_restart-%j.out
#SBATCH --error=/pscratch/sd/m/mpowell/GOAMAZON_LES/logs/goamazon_restart-%j.err
#SBATCH --mail-user=mp4257@columbia.edu
#SBATCH --mail-type=END,FAIL
#SBATCH -A m1266
#
# Restart variant of sbatch_runs.sh (adapted from the CASS restart workflow):
#   - Does NOT delete restart dumps or goamazon.out; does NOT run init.
...
mkdir -p /pscratch/sd/m/mpowell/GOAMAZON_LES/logs

IFS=':' read -ra DIRS <<< "$SIM_DIRS"
echo "Restarting ${#DIRS[@]} goamazon simulations at $(date)"
...
srun -n ${#DIRS[@]} bash -c '
    ...
    endtime=$(awk -F= "/^endtime/{gsub(/[ .]/,\"\",\$2); print \$2}" goamazon.ini)
    ...
    if [[ ! -f "goamazon.ini.before_restart" ]]; then
        cp goamazon.ini "goamazon.ini.before_restart"
    fi
    sed -i "s/^starttime = [0-9.eE+-]*/starttime = ${restart_t}./" goamazon.ini

    grep "^starttime" goamazon.ini
    echo "[GPU $SLURM_LOCALID] Restarting at sim t=${restart_t}s (LT $(awk "BEGIN{printf \"%.2f\", 8.0 + ${restart_t}/3600}"))"

    ./microhh run goamazon
    rc=$?
