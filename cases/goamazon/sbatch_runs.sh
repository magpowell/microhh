...
#SBATCH --output=/pscratch/sd/m/mpowell/GOAMAZON_LES/logs/goamazon-%j.out
#SBATCH --error=/pscratch/sd/m/mpowell/GOAMAZON_LES/logs/goamazon-%j.err
...
echo "Starting ${#DIRS[@]} goamazon simulations at $(date)"
...
srun -n ${#DIRS[@]} bash -c '
    ...
    rm -f goamazon.default.*.nc goamazon.column.*.nc goamazon.out
    rm -f *.[0-9][0-9][0-9][0-9][0-9][0-9][0-9]
    rm -f *.xy.* *_kernel.txt
    ./microhh init goamazon
