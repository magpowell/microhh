...
#SBATCH --output=/pscratch/sd/m/mpowell/GOAMAZON_LES/logs/postproc-%j.out
#SBATCH --error=/pscratch/sd/m/mpowell/GOAMAZON_LES/logs/postproc-%j.err
...
for dir in "${DIRS[@]}"; do
    ...
    echo "=== $dir ($(date)) ==="
    cd "$dir"
    $PY cross_to_nc.py -f goamazon.ini -m xy -n 8 $T0_ARG
    $PY 3d_to_nc.py -f goamazon.ini -v T ql qi qt thl w b qr qs qg \
        -p single -n 8 $T0_ARG
    echo "=== done $dir ($(date)) ==="
done
