...
#SBATCH --output=/pscratch/sd/m/mpowell/ARM97SD_LES/logs/postproc-%j.out
#SBATCH --error=/pscratch/sd/m/mpowell/ARM97SD_LES/logs/postproc-%j.err
...
for dir in "${DIRS[@]}"; do
    ...
    echo "=== $dir ($(date)) ==="
    cd "$dir"
    $PY cross_to_nc.py -f arm97sd.ini -m xy -n 8 $T0_ARG
    $PY 3d_to_nc.py -f arm97sd.ini -v T ql qi qt thl w b qr qs qg \
        -p single -n 8 $T0_ARG
    echo "=== done $dir ($(date)) ==="
done
