...
#SBATCH --output=/pscratch/sd/m/mpowell/ARM97SD_LES/logs/arm97sd-%j.out
#SBATCH --error=/pscratch/sd/m/mpowell/ARM97SD_LES/logs/arm97sd-%j.err
...
echo "Starting ${#DIRS[@]} arm97sd simulations at $(date)"
...
srun -n ${#DIRS[@]} bash -c '
    ...
    rm -f arm97sd.default.*.nc arm97sd.column.*.nc arm97sd.out
    rm -f *.[0-9][0-9][0-9][0-9][0-9][0-9][0-9]
    rm -f *.xy.* *_kernel.txt
    ./microhh init arm97sd
    ./microhh run arm97sd
    status=$?
