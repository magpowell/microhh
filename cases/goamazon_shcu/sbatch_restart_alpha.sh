...
#   - Does NOT delete restart dumps or goamazon_shcu.out; does NOT run init.
...
#
# As with sbatch_runs_alpha.sh, all site-specific sbatch flags (--account,
# --partition, --gres, --time, --output, --error, --job-name) come from the
# submitting script, since #SBATCH cannot expand $SCRATCH.
#
# SIM_DIRS: colon-separated run directories, via --export.

set -uo pipefail

# MICROHH_DIR must come from the submitting script via --export: sbatch copies
# this file into Slurm's spool dir, so BASH_SOURCE cannot locate the repo.
...
echo "Restarting ${#DIRS[@]} goamazon_shcu simulations at $(date)"
...
srun -n ${#DIRS[@]} bash -c '
    ...
    endtime=$(awk -F= "/^endtime/{gsub(/[ .]/,\"\",\$2); print \$2}" goamazon_shcu.ini)
    ...
    if [[ ! -f "goamazon_shcu.ini.before_restart" ]]; then
        cp goamazon_shcu.ini "goamazon_shcu.ini.before_restart"
    fi
    sed -i "s/^starttime = [0-9.eE+-]*/starttime = ${restart_t}./" goamazon_shcu.ini

    grep "^starttime" goamazon_shcu.ini
